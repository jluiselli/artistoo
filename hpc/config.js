...
let config = {
	...
	conf : {
		...
		seed : 2853,							// Seed for random number generation.
		T : 2,								// CPM temperature
	...
	simsettings : { 
		...
		LOGRATE : 50,							// Output stats every <LOGRATE> MCS.
		SAVERATE : 10,
		FLUSHRATE : 30
...
function createOutputs(){
	...
	// Save sim every SIMRATE steps
	if( this.time % this.saverate == 0 ){
		this.saveSim()
	}
}
...
let lastgener = "0"
let lastgenerpath = "./"+config['simsettings']["LOGPATH"]+'/'+"lastgener.txt"
if (fs.existsSync(lastgenerpath)){
	fs.unlinkSync(lastgenerpath)
}
function saveSim(){
		// Save simulation every SAVERATE steps


		// Record last generation
		if( this.time % config['simsettings']['SAVERATE'] == 0 ){
			lastgener = JSON.stringify(this.time)
			lastgener += 
			fs.writeFileSync(lastgenerpath,lastgener)
		}
}

...
for path in dfs: #All individual run one by one
    ...
    # categorical.append({"type": "mito", "death":df[(df['type']== 'mito')].permito.sum()/7000, "setting":dfs[path]['setting']}) ## NO OPTION SETTING FOUND

    categorical.append({"type": "host", "death":df[(df['type']== 'host')].perhost, "seed":dfs[path]['seed']})
...
# # g2 = sns.barplot(data=categorical[(categorical['type'] == "host")], x="setting", y="death", ax=ax[1]) # NO OPTION SETTING. Used to differentiate parameters ?
...
print(keywords.nfile("allcorrectedcat.png"))
